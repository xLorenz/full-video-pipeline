// whoosh — rising band-passed noise sweep ("something arrived").
...
export default async function render(Tone, { rng, params, duration, destination, sr }) {
  const noise = envAttack(fadeOut(noiseArray(rng, duration, sr), 0.18, sr), 0.25, sr);
  const lp = new Tone.Filter({ type: "lowpass", Q: 0.6 });
  lp.frequency.setValueAtTime(350, 0);
  lp.frequency.exponentialRampToValueAtTime(7500, duration);
