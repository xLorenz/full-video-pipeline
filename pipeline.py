...
def run_step_9(title, vdir):
    ...
    Per HyperFrames's "sub-comp must be mounted" rule, every
    `compositions/scene-NN.html` referenced via `data-composition-src` must
    be mounted in `index.html` for it to render independently. We rewrite
    the MainVideo root `index.html`'s scene-layers slot with one
    `<div data-composition-src="compositions/scene-NN.html" ...>` per
    scene before the render loop.

    Lint gate runs once before the loop. If a scene render fails,
    ...
    if not index_path.exists():
        print(f"  ERROR: {index_path} not found — re-run pipeline.py new {title}.")
        return False

    # Rewrite index.html's {{SCENE_LAYERS}} slot with one mounted scene div per
    # scene in scenes.json. Each uses data-composition-src so HyperFrames can
    # both preview the assembled video AND render each scene independently.
    # Durations come from `actual_duration_seconds` set by Step 6.
    scenes = load_scenes(title)
    scene_ids = sorted(set(s["id"] for s in scenes))
    by_id = {s["id"]: s for s in scenes}

    # Cumulative start times in seconds for `data-start` on each scene div.
    # In HyperFrames, `data-start` is the scene's offset into the parent
    # composition's timeline; `data-duration` is the scene's own length.
    cumulative = 0.0
    layer_lines = []
    for sid in scene_ids:
        s = by_id[sid]
        padded = f"{sid:02d}"
        # Prefer seconds (new schema) but fall back to actual_duration_frames/fps.
        dur = s.get("actual_duration_seconds")
        if dur is None:
            dur_frames = s.get("actual_duration_frames")
            if dur_frames is None:
                print(f"  WARNING: Scene {sid} missing actual_duration — skipping mount")
                continue
            fps = (load_pipeline_config(video_dir=vdir).get("video", {}) or {}).get("fps", 30)
            dur = dur_frames / fps
        dur = float(dur)
    ...
    layers_block = "\n".join(layer_lines)
    if not layers_block.strip():
        layers_block = "    <!-- No scenes in scenes.json yet (Step 6 not run?) -->"

    index_text = index_path.read_text(encoding="utf-8")

    # Total duration across all scenes — the root composition's data-duration.
    total_duration = f"{cumulative:.3f}"

    # Replace the {{SCENE_LAYERS}} placeholder with the generated block.
    if "{{SCENE_LAYERS}}" in index_text:
        index_text = index_text.replace("{{SCENE_LAYERS}}", layers_block)
    else:
        # Already-scaffolded project — replace the existing scene-layers region.
        # The foundation template leaves a tight comment + blank region; the
        # emitter guarantees stable delimiters by matching a start anchor +
        # blank whitespace up to a `<script>` or close tag. We use a regex for
        # robustness: replace the region between the "Scene layers are emitted"
        # marker comment and the closing `-->` if present, or re-inject inside
        # the #main-composition root div.
        import re as _re
        # Try the foundation's marker first.
        marker = r"<!--\s*Scene layers are emitted by pipeline\.py.*?-->"
        if _re.search(marker, index_text, flags=_re.DOTALL):
            index_text = _re.sub(
                marker, f"<!-- Scene layers (auto-generated by pipeline.py run_step_9) -->\n      {layers_block}",
                index_text, flags=_re.DOTALL, count=1
            )
        else:
            # Last-resort fallback: replace the inner contents of #main-composition.
            ...
            if m:
                inner = m.group(2)
                # If the existing inner already contains a `data-composition-src`
                # for `scene-01.html`, we've already run this before; wipe the
                # existing scene layers and re-inject fresh ones.
                inner = _re.sub(
                ...
                print("  WARNING: Could not locate scene-layers region in index.html")

    # Substitute the root composition's total duration.
    index_text = index_text.replace("{{TOTAL_DURATION}}", total_duration)
    # If data-duration="0.0" was already written into the root div by a prior
    # scaffold, replace it with the new value.
    import re as _re2
    root_dur_re = _re2.compile(
        r'(<div[^>]*id="main-composition"[^>]*\sdata-duration=")([^"]+)(")',
        flags=_re.DOTALL | _re.IGNORECASE,
    )
    index_text = root_dur_re.sub(
        lambda m: f'{m.group(1)}{total_duration}{m.group(3)}',
        index_text, count=1
    )

    index_path.write_text(index_text, encoding="utf-8")
    print(f"  Injected {len(layer_lines)} scene-layer div(s) into {index_path.relative_to(vdir.parent)}; "
          f"total duration {cumulative:.3f}s.")
