...
- **`uActive` is an envelope, not a presence flag**: upstream's pointer-active flag (and the lens radius scaling by it) becomes a waypoint envelope — `activePath` is a list of `{ at, v }` stops (activation value over composition progress), smoothstepped between stops. To keep the lens OFF until a trigger moment, hold `v` at 0 up to the trigger and only then step to 1: "nothing until a shatter at 60%" is `[{at:0,v:0},{at:0.6,v:0},{at:0.62,v:1}]` — the earlier-looking `[{at:0,v:0},{at:0.6,v:1}]` would instead GROW the lens in over the whole first 60%. Reforming later just drops the envelope back: append `{at:0.8,v:0}`. `uBase` (baseStrength) still lifts the whole page if you want it.
