...
- **One card, no scroll, decrypts at 3s of a 5s video**: a centered single-card scene on a FLAT backdrop, `lensPath: [{x:0.5,y:0.5,at:0},{x:0.5,y:0.5,at:1}]` (static center), `radius` sized to cover the card (the half-diagonal plus feather — 600 covers an 880×560 card), `background` set to the backdrop color EXACTLY (or `passthrough: 1` for a pure glyph overlay — the canvas stays transparent outside the card either way, because empty cells carry no glyph and the content's alpha there is 0), and `activePath: [{at:0,v:0},{at:0.6,v:0},{at:0.62,v:1},{at:0.74,v:1},{at:0.78,v:0}]` — the card rises in encrypted, decrypts at 60%, holds, and re-locks at 78%.
...
See the optional-preview instructions in [`../README.md`](../README.md). Set `animations_preview_requested: true` in `pipeline_state.json` before running `complete` at Step 8. The preview passes a frame-sized monospace `Dossier` as `children` (shape-matched cipher needs glyph-dense content).

For a single "secret message" card, center it on a flat dark backdrop matching `background` exactly, with a static center `lensPath`, `radius: 600`, and the hold-off `activePath` from the recipe above.
