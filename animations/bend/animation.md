...
| `fadeInFrames` / `fadeOutFrames` | 0 / 0 | Treatment fade, frames. The fold amounts grow in with the fade-in and shrink with the fade-out (eased), so the video does not open on a fully folded edge crossfading over the flat DOM |
